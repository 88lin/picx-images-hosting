...
(function () {
    ...
    const BACKUP_HOSTS = [
        { name: "imgdd.com", url: "https://imgdd.com/api/1/upload", field: "source", extract: j => j?.image?.url || j?.image?.display_url },
        { name: "tucdn.wpon.cn", url: "https://tucdn.wpon.cn/api/upload", field: "image", extract: j => j?.data?.url || j?.url },
        { name: "share-text.org", url: "https://share-text.org/api/image-to-url", field: "file", extract: j => j?.url || j?.data?.url },
    ];

    function uploadToBackupHost(host, blob, filename) {
        return new Promise((resolve, reject) => {
            const formData = new FormData();
            formData.append(host.field, blob, filename);
            GM_xmlhttpRequest({
                method: "POST",
                url: host.url,
                data: formData,
                onload: (res) => {
                    try {
                        ...
                        const url = host.extract(json);
                        if (url) resolve(url);
                        else reject(new Error(host.name + " 未返回链接"));
                    } catch (e) {
                        reject(new Error(host.name + " 解析失败"));
                    }
                },
                onerror: () => reject(new Error(host.name + " 网络错误")),
            });
        });
    }

    async function tryBackupHosts(blob, filename) {
        for (const host of BACKUP_HOSTS) {
            try {
                const url = await uploadToBackupHost(host, blob, filename);
                return { url, name: host.name };
            } catch (e) {
                continue;
            }
        }
            ...
            onload: async (res) => {
                try {
                    ...
                    if (imageUrl) {
                        ...
                        const backup = await tryBackupHosts(blob, filename);
                        hideImagePreview();
                        if (backup) {
                            showNotification(`上传成功（备用：${backup.name}）`, "success");
                            insertRawUrl(backup.url, targetElement, filename);
                        } else {
        ...
        xhr.onload = function () {
            try {
                ...
                if (imageUrl) {
                    ...
                    tryBackupHosts(file, filename).then(backup => {
                        if (backup) resolve(backup.url);
                        else reject(new Error("上传成功但未返回链接"));
                    }).catch(() => reject(new Error("上传成功但未返回链接")));
                }
            } catch (e) {
                tryBackupHosts(file, filename).then(backup => {
                    if (backup) resolve(backup.url);
                    else reject(new Error("解析响应失败：" + e.message));
                }).catch(() => reject(new Error("解析响应失败：" + e.message)));
            }
        };
        xhr.onerror = function () {
            tryBackupHosts(file, filename).then(backup => {
                if (backup) resolve(backup.url);
                else reject(new Error("上传失败（网络错误）"));
            }).catch(() => reject(new Error("上传失败（网络错误）")));
        };
        xhr.send(formData);
    });
};
