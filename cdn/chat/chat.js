...
`;
            ...
            function H(t, quiet) {
                ...
                t.msg = t.msg.replace(/【(.*?)】/g, function (match, p1) {
                    if (/^(eveonecat-animated|eveonecat-static|meiji-dashu|mochadandan|onion|onion-animated|onion-static|popo|shuitunlulu|wangdan|xiaohuangtun|xiaokumao|xiaoxiongchong|yantuanzi|yier-bubu|yuexinmiao)\//.test(p1)) {
                        // Smoji 大图包（含文字，需放大）
                        return `<a><img src="https://s3-cdn.zsh.moe/smoji/${p1}.webp" alt="${p1}" style="max-width: 6rem;"></a>`;
                    } else if (/^(daimaobatiao|douyin-current|douyin-limited|it-home|xiaohongshu)\//.test(p1)) {
                        // Smoji 常规包
...
(function () {
    ...
    let hoverTimer = null;
    let openedAt = 0;
    // 只有下面选 tutu 那一次放行，见 item 的 click。
    let allowPup = false;

    function show() {
        const rect = pupBtn.getBoundingClientRect();
        dropdown.style.display = "block";
        // 宽度对齐按钮；内容比按钮宽时（手机上按钮只剩图标 28px）按内容走，然后左右居中。
    ...
    // 按钮身上挂着 data-chevereto-pup-trigger，直接点会被第三方脚本接走、弹一个图床窗口。
    // 它只该当菜单用：在 window 捕获阶段截下这一下（捕获比按钮自己的监听器先跑，
    // stopPropagation 之后事件到不了按钮），改成开合菜单。手机上没有 hover，也全靠这条。
    window.addEventListener("click", function (e) {
        ...
        if (e.target.closest("[data-chevereto-pup-trigger]")) {
            ...
            e.preventDefault();
            e.stopPropagation();
            // 触屏上一次 tap 会先合成 mouseenter（已经 show 过），400ms 内的这下不算"再点一次"。
    ...
    dropdown.querySelectorAll("[data-img-host]").forEach(item => {
        ...
        item.addEventListener("click", () => {
            ...
            if (!url) return;
            if (hostKey === "tutu") {
                // tutu 这条路本来就是借第三方脚本上传的，只有这一下放行给它。
                allowPup = true;
                pupBtn.click();
                allowPup = false;
            } else {
