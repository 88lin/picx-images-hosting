const resultState = {
      ...
      function renderAnimalArchetype(animalName) {
       ...
       function thinkingText() {
          ...
          let base = `灵活性（${agi}）决定了你的视野宽度，能看见多少条路，力量感（${str}）决定你能在一条路上走多远。真正的效率，源于在两者间找到动态平衡。`;
          ...
          return base + `你整体偏向平衡型：既不因过早锁死方向而错失机会，也不因无限发散而消耗精力。为不同类型的决策设定不同节奏规则，是你进一步提升判断力的关键。`;
        }
        function emotionText() {
          const sec = Math.round(score.SEC);
          const sol = Math.round(score.SOL);
          let text = `在你的情感模式中，安全感影响你对不确定性的敏感程度，而社交性决定你更倾向向外确认，还是先在内部消化。`;
          if (L.STR === 'high') {
            text += '你的力量感较强，这意味着即便情绪被触发，你通常也能维持表面的稳定，不太容易当场失控。';
          } else if (L.COM === 'high') {
            text += '你较强的配合倾向，往往让你在情绪出现时先顾全关系，而不是立刻表达自己的感受。';
          } else if (L.DOM === 'high') {
            text += '你在情绪被触发时，更容易直接表达立场，而不是选择退让或回避。';
          } else if (L.AGI === 'high') {
            text += '你的思维灵活性较高，情绪出现后容易在不同解释之间来回切换，既有助于理解，也可能带来额外消耗。';
          } else if (L.AES === 'high') {
            text += '你对氛围与感受较为敏感，情绪往往会被环境细节放大。';
          }
          if (L.SEC === 'high' && L.SOL === 'low') {
            text += `当面临信息模糊或边界不清时，你容易陷入内心的反复推演。你的优势是谨慎细致，但代价是压力容易内化。对你而言，把担忧转成可确认的问题，往往比单纯安抚更有效。`;
            ...
            text += `你更习惯独立消化情绪，对环境变化不易被击穿。但也别把自己封闭起来，偶尔找个信任的朋友吐吐槽，也是给心灵通通风。`;
          } else {
        ...
        function growthText() {
          ...
          return `优先看低分维度带来的“盲区风险”。当前相对较低的是${dimLabel[a]}（${lowA}）与${dimLabel[b]}（${lowB}）。建议你重点实践两条规则：① ${adviceMap[a]}② ${adviceMap[b]}把注意力放在关键场景的改进，而不是追求面面俱到。`;
        }
