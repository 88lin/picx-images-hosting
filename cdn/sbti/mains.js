!function(){var e=(window.location&&window.location.hostname||"").toLowerCase(),n=new Set(["pro-sdz6y0uh.maozi.io","nuaa.tech","pro.ccwu.cc","88lin.eu.org","link.3ceng.cn"]),t=n.has(e)||e.endsWith(".nuaa.tech")||e.endsWith(".88lin.eu.org");if(!t)throw new Error;var o=!1,a=[];function i(e,n,t,o){if(!e||!e.addEventListener)return function(){};e.addEventListener(n,t,o);var i=function(){try{e.removeEventListener(n,t,o)}catch(e){}};return a.push(i),i}function r(){if(o)return;o=!0;try{a.forEach(function(e){e()})}catch(e){}try{window.setTimeout=function(){} }catch(e){}try{window.setInterval=function(){} }catch(e){}try{window.requestAnimationFrame=function(){} }catch(e){}try{window.queueMicrotask=function(){} }catch(e){}try{var n=function(e){return e&&e.preventDefault&&e.preventDefault(),e&&e.stopImmediatePropagation&&e.stopImmediatePropagation(),!1};["click","mousedown","mouseup","keydown","keyup","keypress","contextmenu","touchstart","touchend","pointerdown","pointerup","wheel"].forEach(function(t){i(window,t,n,{capture:!0}),i(document,t,n,{capture:!0})})}catch(e){}}i(window,"keydown",function(e){var n=(e.key||"").toLowerCase(),t=e.keyCode||e.which,a=/mac/i.test(navigator.platform);if("f12"===n||123===t)return e.preventDefault(),e.stopImmediatePropagation&&e.stopImmediatePropagation(),void r();if((e.ctrlKey||a&&e.metaKey)&&e.shiftKey&&("i"===n||"j"===n||"c"===n||73===t||74===t||67===t))return e.preventDefault(),e.stopImmediatePropagation&&e.stopImmediatePropagation(),void r();if((e.ctrlKey||a&&e.metaKey)&&("u"===n||85===t))return e.preventDefault(),e.stopImmediatePropagation&&e.stopImmediatePropagation(),void r()},{capture:!0}),i(window,"contextmenu",function(e){var n=e.target,t=(n&&n.tagName||"").toLowerCase(),o="input"===t||"textarea"===t||"select"===t||n&&n.isContentEditable,a=!!(window.getSelection&&window.getSelection().toString());o||a||(e.preventDefault(),e.stopPropagation&&e.stopPropagation())},{capture:!0});var c=160,d=setInterval(function(){if(o)return void clearInterval(d);var e=performance.now();try{debugger}catch(e){}performance.now()-e>c&&(clearInterval(d),r())},1e3);i(document,"visibilitychange",function(){document.hidden?function(){try{clearInterval(d)}catch(e){}}():o||(function(){try{clearInterval(d)}catch(e){}d=setInterval(function(){if(o)return void clearInterval(d);var e=performance.now();try{debugger}catch(e){}performance.now()-e>c&&(clearInterval(d),r())},1e3)}())})}();
  ...
  "DRUNK": {
    ...
    const CDN_BASE = 'https://cdn.jsdmirror.com/gh/88lin/picx-images-hosting@master/sbti';
    const TYPE_IMAGES = {
      'IMSB': `${CDN_BASE}/IMSB.png`,
      'BOSS': `${CDN_BASE}/BOSS.png`,
      'MUM': `${CDN_BASE}/MUM.png`,
      'FAKE': `${CDN_BASE}/FAKE.png`,
      'Dior-s': `${CDN_BASE}/Dior-s.png`,
      'DEAD': `${CDN_BASE}/DEAD.png`,
      'ZZZZ': `${CDN_BASE}/ZZZZ.png`,
      'GOGO': `${CDN_BASE}/GOGO.png`,
      'FUCK': `${CDN_BASE}/FUCK.png`,
      'CTRL': `${CDN_BASE}/CTRL.png`,
      'HHHH': `${CDN_BASE}/HHHH.png`,
      'SEXY': `${CDN_BASE}/SEXY.png`,
      'OJBK': `${CDN_BASE}/OJBK.png`,
      'JOKE-R': `${CDN_BASE}/JOKE-R.png`,
      'POOR': `${CDN_BASE}/POOR.png`,
      'OH-NO': `${CDN_BASE}/OH-NO.png`,
      'MONK': `${CDN_BASE}/MONK.png`,
      'SHIT': `${CDN_BASE}/SHIT.png`,
      'THAN-K': `${CDN_BASE}/THAN-K.png`,
      'MALO': `${CDN_BASE}/MALO.png`,
      'ATM-er': `${CDN_BASE}/ATM-er.png`,
      'THIN-K': `${CDN_BASE}/THIN-K.png`,
      'SOLO': `${CDN_BASE}/SOLO.png`,
      'LOVE-R': `${CDN_BASE}/LOVE-R.png`,
      'WOC!': `${CDN_BASE}/WOC.png`,
      'DRUNK': `${CDN_BASE}/DRUNK.png`,
  ...
  "So3": {
    ...
    const SBTI_HISTORY_KEY = 'sbtiResultHistory';
    const SBTI_HISTORY_LIMIT = 10;
    const SBTI_NOTICE_DISMISSED_KEY = 'sbtiFreeNoticeDismissed';

    const app = {
      shuffledQuestions: [],
      answers: {},
      previewMode: false,
      guidedFillMode: false,
      activeBrowseTypeCode: null
    };
    ...
    const browseScreen = document.getElementById('browse');
    const browseDetailPanel = document.getElementById('browseDetailPanel');

    function initNoticeBanner() {
      if (!noticeBanner) return;

      const dismissed = localStorage.getItem(SBTI_NOTICE_DISMISSED_KEY) === 'true';
      if (!dismissed) {
      ...
      if (noticeClose) {
        noticeClose.addEventListener('click', () => {
          ...
          localStorage.setItem(SBTI_NOTICE_DISMISSED_KEY, 'true');
        });
      }
    }
    ...
    function setBrowseMobileMode(mode) {
      browseScreen.classList.remove('mobile-list-mode', 'mobile-detail-mode');
      if (!isMobileBrowseLayout()) return;
      browseScreen.classList.add(mode === 'detail' ? 'mobile-detail-mode' : 'mobile-list-mode');
    }

    function scrollBrowseIntoView(target) {
      if (!target) return;
      target.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    function getBrowsableTypes() {
      const normalTypes = NORMAL_TYPES.map((type) => ({
        ...type,
        ...TYPE_LIBRARY[type.code],
        special: false
      }));

      const specialTypes = ['DRUNK', 'HHHH'].map((code) => ({
        ...TYPE_LIBRARY[code],
        special: true,
        pattern: null
      }));
    ...
    function getBrowsableType(code) {
      return getBrowsableTypes().find((type) => type.code === code) || null;
    }

    function renderBrowseDimList(type) {
      const browseDimTitle = document.getElementById('browseDimTitle');
      const browseDimList = document.getElementById('browseDimList');

      if (type.special || !type.pattern) {
        browseDimTitle.textContent = '特殊人格说明';
        const specialNotes = {
          DRUNK: {
            title: '隐藏触发人格',
            body: 'DRUNK 不走标准 15 维匹配，只要饮酒隐藏题被特定选项命中，系统就会直接判给这个人格。它本质上是作者埋的彩蛋型人格。'
          },
          HHHH: {
            title: '系统兜底人格',
            body: 'HHHH 会在标准人格库最高匹配度低于 60% 时出现。不是你没有人格，而是你的脑回路把标准人格库整不会了。'
          }
        };
        const note = specialNotes[type.code] || {
          title: '特殊人格',
          body: '这个人格不按标准 15 维模式展示。'
        };
        browseDimList.innerHTML = `
          <div class="dim-item">
            <div class="dim-item-top">
              <div class="dim-item-name">${note.title}</div>
              <div class="dim-item-score">特殊机制</div>
            </div>
            <p>${note.body}</p>
          </div>
        `;
      ...
      browseDimTitle.textContent = '十五维度特征签名';
      const levels = parsePattern(type.pattern);
      browseDimList.innerHTML = dimensionOrder.map((dim, index) => {
        const level = levels[index];
        return `
          <div class="dim-item">
            <div class="dim-item-top">
              <div class="dim-item-name">${dimensionMeta[dim].name}</div>
              <div class="dim-item-score">${level}</div>
            </div>
    ...
    function renderBrowseDetail(typeCode, options = {}) {
      const type = getBrowsableType(typeCode);
      if (!type) return;
      const { activateMobileDetail = true } = options;

      app.activeBrowseTypeCode = type.code;
      document.querySelectorAll('.browse-item').forEach((item) => {
        item.classList.toggle('active', item.dataset.typeCode === type.code);
      });

      const posterBox = document.getElementById('browsePosterBox');
      const posterImage = document.getElementById('browsePosterImage');
      const posterCaption = document.getElementById('browsePosterCaption');
      const imageSrc = TYPE_IMAGES[type.code];

      document.getElementById('browseModeKicker').textContent = type.special ? '特殊人格档案' : '标准人格档案';
      document.getElementById('browseTypeName').textContent = `${type.code}（${type.cn}）`;
      document.getElementById('browseBadge').textContent = type.special ? '特殊人格' : '标准人格';
      document.getElementById('browseTypeSub').textContent = type.special
        ? '这个人格不通过标准 15 维常规匹配产生，属于隐藏触发或系统兜底分支。'
        : `标准模式签名：${type.pattern}`;
      document.getElementById('browseDesc').textContent = type.desc;
      document.getElementById('browseNote').textContent = type.special
        ? '特殊人格会显示机制说明，而不是标准 15 维签名。它们依然属于 SBTI 人格库的一部分。'
        : '上面展示的是该人格在标准人格库里的理论 15 维签名，用于帮助你快速理解这个人格的大致气质。';
      ...
      renderBrowseDimList(type);

      if (activateMobileDetail && isMobileBrowseLayout()) {
        setBrowseMobileMode('detail');
        requestAnimationFrame(() => {
          scrollBrowseIntoView(browseDetailPanel);
        });
      }
    }

    function renderBrowseList() {
      const browseTypes = getBrowsableTypes();
      browseList.innerHTML = '';

      browseTypes.forEach((type) => {
        const item = document.createElement('button');
        item.className = 'browse-item';
        item.type = 'button';
        item.dataset.typeCode = type.code;
        item.innerHTML = `
          <div class="browse-item-top">
            <div class="browse-item-type">${type.code}（${type.cn}）</div>
            <div class="type-tag${type.special ? ' special' : ''}">${type.special ? '特殊人格' : '标准人格'}</div>
          </div>
          <div class="browse-item-intro">${type.intro}</div>
          <div class="browse-item-sub">${type.special ? '特殊机制人格，可直接查看说明。' : `理论签名：${type.pattern}`}</div>
        `;
        item.addEventListener('click', () => {
          renderBrowseDetail(type.code);
        });
        browseList.appendChild(item);
      });
    }

    function openBrowseScreen(initialCode = 'CTRL', options = {}) {
      const { showDetailOnMobile = false } = options;
      renderBrowseList();
      renderBrowseDetail(initialCode, { activateMobileDetail: false });
      showScreen('browse');
      if (isMobileBrowseLayout()) {
        setBrowseMobileMode(showDetailOnMobile ? 'detail' : 'list');
    ...
    function backToBrowseList() {
      setBrowseMobileMode('list');
      requestAnimationFrame(() => {
        scrollBrowseIntoView(browseList);
      });
    }
    ...
    function applyEmptyHistoryState() {
      ...
      const resultTieNote = document.getElementById('resultTieNote');
      const resultTieText = document.getElementById('resultTieText');
      const resultTieTags = document.getElementById('resultTieTags');

      document.getElementById('resultModeKicker').textContent = '历史记录';
      document.getElementById('resultTypeName').textContent = '还没有测试记录';
      document.getElementById('matchBadge').textContent = '先测一次再回来看看';
      document.getElementById('resultTypeSub').textContent = '这里会展示你最近 10 次的 SBTI 结果，点历史项就能重新查看。';
      resultTieText.textContent = '';
      resultTieTags.innerHTML = '';
      resultTieNote.hidden = true;
      document.getElementById('resultDesc').textContent = '当前设备里还没有保存任何测试结果。先做一次测试，系统就会自动把结果写进历史记录，方便你以后直接回看。';
      document.getElementById('posterCaption').textContent = '你还没被拿捏，先去测一次。';
      document.getElementById('funNote').textContent = '历史记录只保存在当前浏览器本地。清缓存、换设备或换浏览器之后，之前的记录可能就看不到了。';
      posterImage.removeAttribute('src');
      posterImage.alt = '';
      posterBox.classList.add('no-image');
    ...
    function loadSbtiHistory() {
      try {
        const raw = localStorage.getItem(SBTI_HISTORY_KEY);
        if (!raw) return [];
    ...
    function saveSbtiHistory(history) {
      try {
        localStorage.setItem(SBTI_HISTORY_KEY, JSON.stringify(history));
      } catch (error) {
        // Ignore storage failures so the test flow still works.
      }
    }
    ...
    function buildHistoryEntry(result) {
      ...
      return {
        ...
        tieTypeCodes: result.tieTypeCodes || [],
        rawScores: result.rawScores,
        levels: result.levels
      };
    }

    function appendSbtiHistoryEntry(result) {
      const entry = buildHistoryEntry(result);
      const nextHistory = [entry, ...loadSbtiHistory()].slice(0, SBTI_HISTORY_LIMIT);
      saveSbtiHistory(nextHistory);
      return nextHistory;
    }

    function getTieNote(result) {
      if (result.special) return '';
      const tieTypeCodes = Array.isArray(result.tieTypeCodes) ? result.tieTypeCodes : [];
      if (tieTypeCodes.length <= 1) return '';

      const tieNames = tieTypeCodes
        .map((code) => TYPE_LIBRARY[code])
        .filter(Boolean)
        .map((type) => `${type.code}（${type.cn}）`);

      if (tieNames.length <= 1) return '';

      return `标准人格里有 ${tieNames.length} 个类型与你并列第一：${tieNames.join('、')}。系统会按固定顺序展示第一个，说明你正好卡在这几个类型的交界处，不是随机乱跳。`;
    }

    function renderTieTags(result) {
      const resultTieTags = document.getElementById('resultTieTags');
      if (!resultTieTags) return;

      resultTieTags.innerHTML = '';
      const tieTypeCodes = Array.isArray(result.tieTypeCodes) ? result.tieTypeCodes : [];
      const currentTypeCode = result.finalTypeCode || (result.finalType && result.finalType.code) || '';
      if (result.special || tieTypeCodes.length <= 1) return;

      tieTypeCodes.forEach((code) => {
        const type = TYPE_LIBRARY[code];
        if (!type) return;
        const tag = document.createElement('button');
        tag.type = 'button';
        tag.className = `type-meta-tag${code === currentTypeCode ? ' is-current' : ''}`;
        tag.dataset.typeCode = code;
        if (code === currentTypeCode) {
          tag.setAttribute('aria-current', 'true');
          tag.title = '当前主结果';
        }
        tag.textContent = `${type.code}（${type.cn}）`;
        resultTieTags.appendChild(tag);
      });
    }

    function applyResultData(result) {
      const type = TYPE_LIBRARY[result.finalTypeCode] || TYPE_LIBRARY[result.finalType.code];
      if (!type) return;
      const tieNote = getTieNote(result);
      const resultTieNote = document.getElementById('resultTieNote');
      const resultTieText = document.getElementById('resultTieText');
      ...
      resultTieText.textContent = tieNote;
      renderTieTags(result);
      resultTieNote.hidden = !tieNote;
    ...
    function renderHistoryList(history = loadSbtiHistory()) {
      historyList.innerHTML = '';
      historyEmpty.style.display = history.length ? 'none' : 'block';
      clearHistoryBtn.style.display = history.length ? 'inline-flex' : 'none';

      history.forEach((entry) => {
        const type = TYPE_LIBRARY[entry.finalTypeCode];
        if (!type) return;
        ...
        item.innerHTML = `
          <div class="history-item-top">
            <div class="history-item-type">${type.code}（${type.cn}）</div>
            <div class="history-item-time">${formatHistoryTime(entry.createdAt)}</div>
          </div>
          <div class="history-item-badge">${entry.badge}</div>
          <div class="history-item-sub">${entry.sub}</div>
        `;
        item.addEventListener('click', () => {
          applyResultData(entry);
          showScreen('result');
        });
    ...
    function openHistoryScreen() {
      const history = loadSbtiHistory();
      renderHistoryList(history);
      if (history.length) {
        applyResultData(history[0]);
      } else {
    ...
    function clearFocusedUnansweredQuestion() {
      questionList.querySelectorAll('.question.unanswered-focus').forEach((card) => {
        card.classList.remove('unanswered-focus');
      });
    }

    function findFirstUnansweredQuestion() {
      const visibleQuestions = getVisibleQuestions();
      return visibleQuestions.find((q) => app.answers[q.id] === undefined) || null;
    }

    function focusUnansweredQuestion(questionId) {
      clearFocusedUnansweredQuestion();
      const target = questionList.querySelector(`[data-question-id="${questionId}"]`);
      if (!target) return;
      target.classList.add('unanswered-focus');
      target.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    function continueGuidedFillIfNeeded() {
      if (!app.guidedFillMode) return;

      const nextUnanswered = findFirstUnansweredQuestion();
      if (nextUnanswered) {
        testHint.textContent = '继续，下一道漏选题也帮你定位好了。';
        focusUnansweredQuestion(nextUnanswered.id);
        return;
      }

      app.guidedFillMode = false;
      clearFocusedUnansweredQuestion();
      renderResult();
    }
    ...
    function handleSubmit() {
      ...
      if (firstUnanswered) {
        app.guidedFillMode = true;
        testHint.textContent = '还有题没做，已经帮你定位到第一道漏选题。';
        focusUnansweredQuestion(firstUnanswered.id);
        return;
      }
    ...
    function computeResult() {
      ...
      const ranked = NORMAL_TYPES.map((type, index) => {
        ...
        return { ...type, ...TYPE_LIBRARY[type.code], distance, exact, similarity, order: index };
      }).sort((a, b) => {
      ...
      const tiedNormals = ranked.filter((type) => (
        type.distance === bestNormal.distance
        && type.exact === bestNormal.exact
        && type.similarity === bestNormal.similarity
      ));
      ...
      let tieTypeCodes = tiedNormals.map((type) => type.code);
      ...
      if (drunkTriggered) {
        ...
        if (tieTypeCodes.length > 1) {
          sub = '你和多个标准人格并列第一，系统已按固定顺序展示主结果。';
          badge = `并列第一 ${tieTypeCodes.length} 个 · 匹配度 ${bestNormal.similarity}% · 精准命中 ${bestNormal.exact}/15 维`;
        }
      }
    ...
    function renderResult() {
      ...
      applyResultData(result);
      renderHistoryList(appendSbtiHistoryEntry(result));
      showScreen('result');
    }
    ...
    document.getElementById('viewHistoryBtn').addEventListener('click', openHistoryScreen);
    document.getElementById('browseTypesBtn').addEventListener('click', () => openBrowseScreen('CTRL'));
    ...
    document.getElementById('browseBackHomeBtn').addEventListener('click', () => showScreen('intro'));
    document.getElementById('browseBackToListBtn').addEventListener('click', backToBrowseList);
    document.getElementById('browseStartBtn').addEventListener('click', () => startTest(false));
    document.getElementById('resultTieNote').addEventListener('click', (event) => {
      const target = event.target.closest('[data-type-code]');
      if (!target) return;
      openBrowseScreen(target.dataset.typeCode, { showDetailOnMobile: true });
    });
