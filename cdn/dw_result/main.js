const resultState = {
      ...
      function renderDimensions(tScores) {
        ...
        const dimensionDescriptions = {
          ...
          'COM': {
            'low': '你与人保持着清晰的边界，不太依赖群体的认同。你更相信自己的判断，这让你与众不同，但有时也会显得难以接近。你是否曾因为不愿迁就而错过一些关系？',
            'medium': '你能在坚持自我和融入群体之间找到平衡。你乐于配合，但不会盲目附和。这种分寸感让你既保持了个性，又能被集体接纳。',
            'high': '你天然亲和，容易让人放下戒备。你在意集体的氛围，愿意为共同目标让渡一部分自我。这份温度很珍贵，但也请记得：你的需求同样重要。'
          },
          'SOL': {
            'low': '你从人群中获得能量，独处久了反而会觉得空落。你善于建立和维护关系，在热闹里状态最好。但在喧嚣之后，偶尔安静下来审视自己，或许同样重要。',
            'medium': '你在独处和相处之间找到了舒适的平衡点。你享受与人交往的乐趣，也珍视自己的私人空间。这种平衡让你既能从关系中获取能量，又能保持内心的独立与宁静。',
            'high': '你享受独处的时光，过多的社交对你是一种消耗。你的世界丰富而深邃，不需要外界的认可来证明自己。但偶尔的真诚连接，或许能为你打开另一个精彩的世界。'
          },
          'AGI': {
      ...
      function renderFeatureCombinations(tScores) {
        ...
        const dimensionPairs = [
          ...
          { pair: ['SOL', 'SEC'], analysis: compareSolitudeAndSecurity },
      ...
      function compareSolitudeAndSecurity(solScore, secScore, solName, secName) {
        // 两个维度各分高/中/低三档，九种组合都要有对应说法。
        // 原来只写了三个角落，剩下全落进"较为平衡"的兜底——两项都很低的人
        // 也会被说成"切换自如"，实测约一成用户会读到这句不准的话。
        const solHigh = solScore > 60, solLow = solScore < 40;
        const secHigh = secScore > 60, secLow = secScore < 40;
        if (solHigh && secHigh) {
          return `你需要独处，也需要确定性。一个安静、不被频繁打扰的空间，是你恢复状态的必要条件。你的关系不多，但每一段都经得起时间。`;
        } else if (solHigh && secLow) {
          return `你享受独处，却不惧怕未知。你愿意一个人走进陌生的领域，把不确定当成有趣而非威胁。这种组合让你在需要独立判断的场合格外自如。`;
        } else if (solLow && secHigh) {
          return `你从人群中获得能量，同时重视稳定。熟悉的圈子和可预期的节奏让你安心，你更愿意把精力投入到长期的关系里，而不是不断重建。`;
        } else if (solLow && secLow) {
          return `你既需要人群，也不怕变化。新环境和新面孔对你不是压力而是刺激，你更容易在流动的关系与场景里找到状态。要留意的是，当节奏排得太满时，也该给自己留一点缓冲。`;
        } else if (solHigh) {
          return `你明显需要属于自己的时间，对确定性的要求则有弹性。只要能保住独处的空间，你对外部变化的容忍度其实不低。`;
        } else if (solLow) {
          return `你倾向于待在人群里，独处久了容易觉得没着落。你对确定性的需求不算突出，真正在意的是身边有没有人，而不是环境稳不稳定。`;
        } else if (secHigh) {
          return `你对确定性的需求比较明确，独处或相处则都能适应。可预期的节奏和清晰的边界，比社交形式本身更能让你安心。`;
        } else if (secLow) {
          return `你对不确定有较高的容忍度，独处或相处都不构成负担。这种松弛让你在变化中不容易慌，但也要留意别把风险估得太轻。`;
        } else {
          return `你在独处与相处之间切换自如，也能根据情境调节对确定性的需求。这种弹性让你既享受得了安静，也应付得来热闹。`;
        }
      }
      ...
      function analyzeCoreMotivations(tScores, dimensionNames) {
        ...
        if (top1[0] === 'DOM') {
          ...
          motivations.push(`你的核心动力来自于拥有属于自己的空间。不被打扰的时间让你思考得更清楚，你更愿意独自把一件事想透、做成，而不是在持续的互动中被牵着走。`);
        } else if (top1[0] === 'AGI') {
        ...
        if (top1[1] - top2[1] < 20) {
          if (top2[0] === 'DOM' && top1[0] !== 'DOM') {
            ...
            motivations.push(`同时，你也需要一定的独处时间来消化和沉淀。这让你在投入外部事务的同时，始终保留一块属于自己的清醒地带。`);
      ...
      function renderAnimalArchetype(animalName) {
        const archetypeData = {
          ...
          '狼': {
            ...
            ethology: '狼群其实是以一对繁殖亲代为核心的家族群体——早年流行的“alpha 狼靠争斗夺权”之说，已被最初提出它的研究者本人推翻。它们的协作狩猎策略展示了高度的智商和团队精神。狼的耐力和毅力使其能够在极端环境中生存。它们通过复杂的沟通系统维护群体和谐，这种社会性和协作能力是其生存的关键优势。'
          },
          '狐': {
          ...
          '狮': {
            ...
            ethology: '狮子是唯一真正群居的猫科动物。狮群的稳定核心是一群有血缘关系的母狮，雄狮则以联盟形式阶段性地守卫领地——威严的外表背后，是一套分工明确的协作机制。狮子的自信和存在感使其在草原生态系统中处于顶端位置。它们的耐心和爆发力在捕猎中展现出完美结合。'
          },
          '熊': {
            mythology: '在北美原住民文化中，熊被视为力量和治愈的象征；在北欧传统中，“狂战士”一词本义即“披熊皮者”，熊象征进入战斗状态的蛮力；在中国传统文化中，熊象征着力量和勇气；在许多文化中，熊也与冬眠和重生的概念相关联。',
            famousPeople: '代表人物包括乔治·华盛顿（美国第一任总统，稳重而有力量）、列夫·托尔斯泰（文学巨擘，思想深邃如熊）、安吉丽娜·朱莉（展现出熊般的母性本能和保护欲）。',
            ethology: '熊是力量与耐力的象征，能够在各种气候条件下生存。它们的杂食性使其食物来源广泛，增强了适应能力。熊的冬眠行为展示了其生理适应性和资源管理能力。虽然平时温和，但在保护幼崽或领地时会爆发出惊人的力量。'
          },
          '兔': {
          ...
          '天鹅': {
            ...
            ethology: '天鹅是水禽中体型最大的物种之一，以其优美的姿态和终身伴侣制闻名。它们在繁殖季节表现出强烈的领地意识和保护行为。天鹅的飞行能力展示了其力量与优雅的结合。幼天鹅从灰色丑小鸭到白色天鹅的转变成为了蜕变和成长的象征。'
          },
          '鹿': {
            mythology: '在凯尔特神话中，鹿是森林之神的化身，象征着敏捷和神秘；在基督教艺术中，鹿常与灵魂追求联系在一起；在中国传统文化中，鹿象征着长寿和吉祥；在许多文化中，鹿被视为连接自然与超自然世界的使者。',
            famousPeople: '代表人物包括弗吉尼亚·伍尔夫（感受极其细腻的作家）、弗里德里希·席勒（德国诗人，作品充满理想主义色彩）、索菲亚·科波拉（导演，作品风格细腻优雅）。',
          ...
          '鹰': {
            ...
            ethology: '鹰是天空中的顶级捕食者，拥有卓越的视力和飞行能力。它们能够从数千米高空发现地面上的小猎物，这种远见是其生存的关键优势。鹰的爪子和喙强大有力，使其能够捕捉和处理大型猎物。一些鹰类展现出惊人的筑巢能力，建造巨大的巢穴。'
          },
          '乌鸦': {
          ...
          '水豚': {
            ...
            ethology: '水豚是世界上最大的啮齿动物，以其高度社交性和温和性格著称。它们生活在群体中，通过频繁的社交互动维持关系。水豚与许多其他物种，包括鳄鱼，都能和平共处，这种能力在动物界极为罕见。它们的半水生生活方式和特殊的消化系统使其能够有效利用水生植物资源。'
          },
          '鲸': {
            mythology: '在许多海洋文化中，鲸鱼被视为神圣的生物和祖先的化身；中世纪航海传说里有一类“巨鲸如岛”的故事，水手误把鲸背当作陆地登陆；在一些太平洋岛屿文化中，鲸鱼是海洋保护神；在现代文化中，鲸鱼常象征着智慧、神秘和深邃的情感。',
            famousPeople: '代表人物包括列奥纳多·达·芬奇（思想如同海洋般深邃）、卡尔·荣格（心理学大师，探索人类心灵的深度）、玛丽·居里（在科学海洋中探索未知）。',
            ethology: '鲸鱼是地球上最大的哺乳动物，也是最聪明的海洋生物之一。它们拥有复杂的社会结构和沟通系统，能够发出复杂的声音进行长距离交流。鲸鱼的迁徙行为展示了其惊人的导航能力和记忆力。一些鲸鱼种类展现出利他主义行为，会帮助受伤的同伴。'
          },
          '鹦鹉': {
            mythology: '在许多文化中，鹦鹉被视为语言和沟通的象征；亚历山大东征之后，会说话的鹦鹉才传入希腊，一度被当作来自东方的奇珍；在玛雅文明中，金刚鹦鹉是与太阳相关的神圣鸟类；在现代文化中，鹦鹉常象征着模仿、社交和活力。',
            famousPeople: '代表人物包括莎士比亚（语言大师，如同鹦鹉般善于运用语言）、奥普拉·温弗瑞（出色的沟通者和社交达人）、爱因斯坦（善于交流复杂思想）。',
            ethology: '鹦鹉以其出色的语言模仿能力和高智商著称。它们是高度社交的鸟类，生活在复杂的群体结构中。鹦鹉的好奇心和学习能力使其能够适应各种环境。它们的长寿（某些种类可活80年以上）使其能够积累丰富的经验和知识。'
          },
          '章鱼': {
            mythology: '在古希腊神话中，多触手的海怪斯库拉守在墨西拿海峡，令航海者闻之色变；在日本浮世绘与民间传说中，章鱼是神秘而灵巧的水族；近代洛夫克拉夫特笔下的克苏鲁，则让触手成为未知与深渊的象征。',
            famousPeople: '代表人物包括达芬奇（多才多艺，如章鱼有多种能力）、居里夫人（在多个科学领域都有贡献）、爱因斯坦（思维方式灵活多变，如同章鱼的触手探索不同方向）。',
            ethology: '章鱼是海洋中最聪明的无脊椎动物，拥有复杂的神经系统和学习能力。它们的八条触手上布满了吸盘，使其能够操纵物体和捕捉猎物。章鱼能够通过改变颜色和纹理进行伪装，是自然界中的伪装大师。它们的解决问题能力和逃脱技巧令人惊叹。'
          },
          '鲨鱼': {
            ...
            ethology: '鲨鱼是地球上最古老的脊椎动物之一，已经在地球上生存了超过4亿年。它们是完美的捕食机器，拥有流线型的身体、敏锐的感官和高效的呼吸系统。鲨鱼的侧线系统使其能够感知水中的微小振动，帮助定位猎物。它们的咬合力惊人，能够轻松撕碎猎物。'
          },
          '海豚': {
            ...
            ethology: '海豚是高度社会化的海洋哺乳动物，生活在复杂的群体结构中。它们通过复杂的声音和身体语言进行沟通。海豚的智商极高，能够解决复杂问题并展现出自我意识。它们经常进行游戏行为，展现出强烈的好奇心和愉悦感。海豚之间的合作狩猎策略展示了其团队精神。'
          },
          '浣熊': {
            ...
            ethology: '浣熊是高度适应性的哺乳动物，能够在城市和自然环境中生存。它们的前爪极为灵巧，能够操纵物体和打开容器。浣熊是杂食性动物，食物来源广泛，包括水果、坚果、昆虫和小型动物。它们的好奇心和探索精神使其能够发现新的食物来源和栖息地。'
          },
          '猫鼬': {
            ...
            ethology: '猫鼬是高度社会化的小型哺乳动物，生活在大型群体中，有着复杂的社会结构。它们的群体中存在明确的分工，包括哨兵职责。猫鼬的视力极佳，能够发现远处的捕食者。它们的捕食策略包括群体协作，共同对付毒蛇等危险猎物。猫鼬之间的相互梳理和社交行为强化了群体纽带。'
          }
        };
        ...
        function workEnvText() {
          ...
          if (score.AES >= 62 && score.COM <= 45) {
              return "你适合安静、优雅且尊重个人空间的专注型环境。嘈杂的开放式办公区和复杂的人际关系会让你分心。你更需要一个能让你静下心来深度思考、打磨作品的物理空间，以保障产出质量。" + evidence;
          }
          if (score.SOL >= 62) {
              return "你适合能保有独立工作空间的环境。相比持续的会议与协作，你在能独自支配的时间里产出最高。给你清晰的目标和不被频繁打断的节奏，你就能把复杂问题啃下来。" + evidence;
          }
          return "你的适应力较强，适合混合型环境：既有基础制度保障底线，又有适度弹性供你发挥。最理想的状态是“中庸务实”，既不追求极端的自由，也不接受绝对的管控，根据任务灵活调整即可。" + evidence;
        }

        function careerPathText() {
          if (score.DOM >= 58 && score.COM >= 55) {
            return "你更可能在资源协调、目标推进与跨角色协作类任务中体现价值。你擅长在多方诉求中聚焦关键目标，并推动事情向前发展。需要注意的是，在追求效率的同时，适当为共识建立预留空间，有助于降低后期返工风险。";
          }
          if (score.AGI >= 58 && score.AES >= 55) {
            return "你更适合参与新产品探索、创意迭代或策略试验类任务。你对新事物保持敏感，能够快速尝试并调整方向。建议定期将经验沉淀为可复用的方法，以避免能力过度分散。";
          }
          if (score.AGI >= 58 && score.STR >= 55) {
            return "你更容易在高不确定性场景中发挥优势，如对外拓展、谈判或攻坚型任务。你的优势更容易在“行动—反馈—迭代”的循环中被放大；将实战经验系统化总结，会显著提升你的长期成长效率。";
          }
          if (score.SEC >= 58 && score.COM >= 55) {
            return "你更适合流程搭建、质量控制与持续交付类任务。你倾向于通过稳定可靠的执行建立信任，在需要长期一致性的岗位上更容易形成个人优势。";
          }
          if (score.STR >= 62 || (score.STR >= 55 && score.AES >= 52)) {
            return "你更适合在专业深度要求较高的领域持续投入，例如技术攻关、专业咨询或内容创作。长期聚焦于复杂问题的解决，有助于你构建难以替代的专业壁垒。";
          }
          return "你的职业适配范围相对广泛，具备较强的可塑性。通过早期多样化尝试，找到既能产生投入感又能获得成果反馈的方向，再进行持续深化，会更符合你的发展节奏。";
        }

        function emotionText() {
          if (score.SEC >= 62) {
            let t = "你的情感模式更偏向于寻求稳定与信任。在关系中，你对不确定性较为敏感，持续的确认与正向反馈有助于你保持安心与投入。这使你往往表现得可靠而有责任感。";
            if (score.COM >= 62) t += "高质量且稳定的人际互动，能够有效增强你的情感安全感。";
            else if (score.COM <= 35) t += "在独处中整理情绪、恢复秩序，是你应对不安的重要方式。";
            return t;
          }
          if (score.SEC <= 30) {
            let t = "你的情感模式更偏向内部自洽。你不太依赖外部持续的情感支持来维持状态，对环境变化具备较强的适应力。这种情感独立性让你在波动中能保持自主节奏。";
            if (score.COM >= 62) t += "即便如此，你依然重视真诚且无需过度消耗的深度关系。";
            else if (score.COM <= 35) t += "独处往往是你获得情感恢复与内在能量的主要来源。";
            return t;
          }
          if (score.COM >= 62) {
            let t = "你从人际连接中获得较多情感能量。高质量的互动能显著提升你的情绪状态，你也更容易在群体中感知他人情绪并作出回应。";
            if (score.SEC >= 55) t += "稳定而互信的关系，是你建立长期连接的重要基础。";
            else if (score.SEC <= 40) t += "多样而充满活力的社交体验，能持续为你提供新鲜感。";
            return t;
          }
          if (score.COM <= 35) {
            let t = "你的情感模式更偏向内在探索。过多的社交互动可能让你感到消耗，你更倾向于在独处中恢复精力并整理思绪。";
            if (score.SEC >= 55) t += "一个安全、不被频繁打扰的私人空间，对你尤为重要。";
        ...
        function thinkingText() {
          if (score.AGI >= score.STR + 20) {
            return "你的思维更偏向发散探索，擅长从宏观视角进行联想与组合。这有利于创意生成与策略构想，但也可能带来较高的“收敛成本”。建议设定明确的筛选标准，将想法转化为可执行方案。";
          }
          if (score.STR >= score.AGI + 20) {
            return "你的思维属于聚焦型，擅长深度归纳与逻辑推演。这种特质有助于攻克复杂难题，但也可能形成“隧道视野”。在做决策时引入“逆向思考”，听取不同意见，有助于修正认知偏差。";
          }
          if (score.DOM >= 62) {
            return "你的思维高度结果导向，习惯从终局反推路径。这种直觉式决策在需要快速响应的情境下极具效率，但也可能忽略执行层面的风险。建议预先推演失败可能，以提升决策的稳健性。";
          }
          if (score.AES >= 62) {
            return "你的思维偏好整体感知与具象化处理。相比于纯逻辑推导，你对画面感、风格与整体性的判断更为敏锐。在创意领域这是核心优势，但在严谨工作中，需借助结构化工具将直觉转化为可验证的逻辑。";
          }
          return "你拥有灵活的双系统思维能力，既能发散探索也能聚焦深挖。根据任务性质在两种模式间自由切换，是你的核心优势。建议根据场景有意识地调整思维策略：策划时大胆假设，执行时严谨求证。";
        }
        ...
        function growthText() {
          const adviceMap = {
            ...
            COM: '主动建立弱连接。有意识地接触不同角色与视角，既能拓展认知边界，也能让协作更顺畅。',
            SOL: '为自己留出固定的独处时段。不被打扰的深度思考时间，往往比持续在线更能推动关键进展。',
          ...
          const spread = Math.round(top1[1]) - Math.round(low1[1]);
          if (spread < 25) {
